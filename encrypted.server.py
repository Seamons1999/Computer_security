...
class EncryptedLogHandler(logging.Handler):
    def emit(self, record):
        #print(f"{logging.getLogger().handlers[0].formatter.format(logging.LogRecord('root', logging.INFO, '', 0, "", None, None))}")
        log_record = logging.LogRecord('root', logging.INFO, '', 0, "", None, None)
        formatted_message = logging.getLogger().handlers[0].formatter.format(log_record)
        #print(formatted_message)
        message = self.format(record)
        encrypted_message = encrypt_message(formatted_message + message)
        with open("app.log", "a") as log_file:
            log_file.write(encrypted_message.decode() + "\n")

def log_encrypted_message(logger, message):
    logger.info(message)
#ENCRYPTION#
#ENCRYPTION#
...
###Added the function before each log for encryption!
###Added the function before each log for encryption!
...
def handle_client(clients, client_socket, addr, max_conn_limiter, max_threads_per_client, active_clients, lock):
    ...
    try:
        ...
        if client_id == 'InvalidJSON' and delay == 0 and numeric_changes == []:
            log_encrypted_message(logging, f'Connection to {addr} aborted due to false json file')
            return
        
        # Lockout mechanism
        if clients[client_id]['failed_logins'] >= 3:
            log_encrypted_message(logging, f'ID {client_id}: Too many failed logins. Access denied')
            return
        ...
        with lock:
            if client_id not in active_clients:
                ...
                log_encrypted_message(logging, f"ID {client_id} already has {max_threads_per_client} active connections.")
                return
            else:
                active_clients[client_id] += 1
        ...
        if client_id in clients:
            if hash_password(client_msg['password'], clients[client_id]['salt']) != clients[client_id]['password']:
                log_encrypted_message(logging, f'ID {client_id}: Invalid password')
            ...
            log_encrypted_message(logging, f'ID {client_id} - Client already registered')
        else:
            # Check if password is strong enough
            if not is_strong_password(client_msg['password']):
                log_encrypted_message(logging, f'ID {client_id} - Not registered due to weak password')
                response = {'status': 'error', 'message': 'Password too weak'}
                client_socket.send(json.dumps(response).encode('ascii'))
                return
            # Register new client
            salt = bcrypt.gensalt()
            clients[client_id] = {'password': hash_password(client_msg['password'], salt), 'counter': 0, 'salt': salt,
                                  'failed_logins': 0}
            log_encrypted_message(logging, f'ID {client_id}: New client registered')

        for step in numeric_changes:
            clients[client_id]['counter'] += step
            log_encrypted_message(logging, f'ID {client_id} - Counter value updated to {clients[client_id]['counter']}')
            time.sleep(delay)
        ...
        with lock:
            ...
            if active_clients[client_id] == 0:
                ...
                log_encrypted_message(logging, f'ID {client_id}: Account deleted because no active clients')
...
def main():
    ...
    ###ENCRYPTION COCE
    encrypted_handler = EncryptedLogHandler()
    encrypted_handler.setLevel(logging.INFO)
    logging.getLogger().addHandler(encrypted_handler)
